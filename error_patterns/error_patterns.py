...
def error_quantifier(
    x,
    full_correct_value=1,
    full_deletion_value=0,
    full_substitution_value=0.6,
    correct_seg_value=1,
    substitution_seg_value=0.6,
...
def error_patterns_table(input_filepath, score_column=True, resolver=True):
    ...
    error_patterns_df = error_patterns_df.merge(
        error_patterns_series, left_index=True, right_index=True
    )
    error_patterns_df["error_basic"] = error_patterns_df["error_pattern"].apply(
        lambda x: x.split("-")[0]
    )
    if resolver:
        resolved_error_list = []
        counter = 0
        for index, target, actual, pattern in error_patterns_df[
            ["IPA Target", "IPA Actual", "error_pattern"]
        ].itertuples():
            if "other" in pattern:
                resolved_error_list.append(
                    error_pattern_resolver(target, actual, pattern)[0]
                )
            else:
                resolved_error_list.append("")
            counter += 1
        ...
        error_patterns_df["error_pattern"].loc[
            error_patterns_df["resolved_error"] != ""
        ] = error_patterns_df["resolved_error"]
    if score_column:
        error_patterns_df["error_score"] = error_patterns_df["error_pattern"].apply(
            error_quantifier
        )
...
def import_test_cases(test_cases="test_cases.txt"):
    ...
    epenthesis_other = txt[
        txt.index("epenthesis_other:") + 1 : txt.index("other:")
    ]  # Extract section
    epenthesis_other = [
        x.split("	") for x in epenthesis_other
